...
%	T &= \frac12 (\dot{x}^2 + \dot{y}^2) = \frac{1}{2} m l^2 \dot{q}^2\\
%	V &= - mg y = -mgl\cos q 
...
%	H = p \dot{q} - L = \frac{p^2}{m l^2} - m g l \cos q\\
...
%		\dd t U\binom{q}{r} &= U M U^{-1} U \binom{q}{p}\\
%		\dd t \binom{x_1}{x_2} = \begin{pmatrix}
%		\lambda_1 & 0\\ 0 & \lambda_2
%		\end{pmatrix} \binom{x_1}{x_2}
%		\intertext{System entkoppelt}
%		\conseq \dot{x}_i(t) = \lambda_i x_i(t) \cong x_i(t) = A e^{\lambda_i t}
%		\intertext{Lösung bei linearen System klar. Bei allgemeinem System numerisch gut zu handhaben\footnote{Vgl. Runge-Kutta-Verfahren}.}		
%		\end{align*}
\end{beispiel*}
...
\todo{gesamte Zeitabhängigkeit im (????)}
...
Jetzt sie $\{,\}$ eine abstrakte Klammer mit den abstrakte Eigenschaften der Poissonklammer, wie verstehen diese als mathematische Struktur.
...
\paragraph{Begriff} Wie beobachtet man physikalische Phänomene \textbf{relativ} zueinander bewegter Bezugssysteme. Invarianz/Kovarianz physikalischer Observablen und Gesetze.
Bei gleichförmig geradlinig zueinander bewegten Systemen (Inertialsysteme)
\conseq \textbf{speizielle} Relativitätstheorie
Darüber hinaus: \textbf{allgemeine} Relativitätstheorie. Beschleunigungen \conseq Scheinkräfte
\conseq Eigenschaften von Raum und Zeit
\conseq Gravitationskräfte als Eigenschaft von Raum und Zeit
Im folgenden "`nur"' die spezielle Relativitätstheorie.
Wichtiges Kriterium für relativistische Phänomene sind die Geschwindigkeiten aller beteiligten Systeme
$v \ll c$ klassisch (nicht relativistisch)
$v \lesssim c$ spezielle Relativitätstheorie.
Theorie so konstruiert, dass die Newtonsche Mechanik als Grenzfall $v \ll c$ enthalten ist.

\subsection{Bezugssysteme / Inertialsysteme}
\paragraph{Klassische Mechanik}
\begin{itemize}
	\item es gibt Bezugssysteme, die anderen gegenüber ausgezeichnet sond, da in ihnen keine Scheinkräfte wirken. \textit{Intertialsysteme}.
	Möglicherweise gibt es einen \textit{absoluten Raum} (Newton).
	\item Es gibt eine \textit{absolute Zeit}. In der klassischen Physik spricht nichts dagegen.
	\item Genauer: In Inertialsystemen nur Newtons Gesetz $\vec{F} = m \ddotvec{r}$ gilt ohne Hinzunahme von Schein-/Trägheitskräften.
	Ausgangspunkt ist das Inertialsystem $\Sigma$.
	Alle Systeme $\Sigma'$, die sich geradlinig, gleichförmig mit $\vec v$ relativ zu $\Sigma$ bewegen ($\vec{r}' = \vec{r} - \vec{v}t$), sind auch Inertialsysteme.
	\conseq Galilei-Transformation (z.B. $\vec{v} \parallel \hat{z}$)
\end{itemize}
Konsequenz für Lichtwellen, die sich in einem System $\Sigma$ sphärisch ausbreiten.
\begin{align*}
	\Sigma& & \Sigma& \text{per Galilei Transformation}\\
	\dotvec{r} &= c \hat{r}, \hat{r} = \frac{\vec{r}}{|\vec{r}|} & \dotvec{r}' &= \dotvec{r} - \vec{v} = c \hat{r} - \vec{v}   
\end{align*}
Keine sphärischen Wenn $\Sigma'$.
Vorstellung von "`Weltäther"' verknüpft mit absolutem Raum.


Behauptung es gibt einen \textbf{absoluten Raum} in dem der Äther ruht. Hier breitet sich das Licht sphärisch aus.
Experimenteller Test
\subsubsection{Michelson-Morley-Experiment}
Idee: Betrachtung der Bewegung der Erde um die Sonne. Die Erde driftet mit immerhin $v = 30 \frac{\mathrm{km}}{\mathrm{h}}$ um die Sonne durch den Weltäther.
Breitet sich das Licht zu unterschiedlichen Jahreszeiten unterschiedlich aus? Oder kann man die Geschwindigkeit des Äthers relativ zur Sonne messen?
Aufbau: vgl. \href{https://de.wikipedia.org/wiki/Michelson-Morley-Experiment}{Wikipedia}
Grob zusammen gefasst, läuft das Licht über zwei verschiedene Wege, welche senkrecht zu einander legen. Diese beiden Wege sind gleich lang. $\delta S = (S_0 - S_1 - S_0) - (S_0 - S_2 - S_0) = m \lambda$ für eine konstruktive Interferenz.
Laufzeiten $\Delta_{ij}$ unter Berücksichtigung des Ätherwindes $S_0 \to S_1: \Delta_{01} = \frac{l_1}{c - v} \text{~(mit Äther)}$, $S_1 \to S_0: \Delta_{10} = \frac{l_1}{c + v}$ \conseq $S_0 - S_1 - S_0: \Delta_1 = \frac{l_1}{c-v} + \frac{l_1}{c+v} = 2 \frac{l_1}{c} \frac{1}{1 - \frac{v^2}{c^2}}$ (aus Zeichnung) $c\Delta_{20}^2 = l_2^2 + v^2 \Delta_{20}^2$, $c\Delta_{02}^2 = l_2^2 + v^2 \Delta_{02}^2$, $\Delta_{20} = \Delta_{02} = \Delta_{2} = 2 \frac{2l_2}{c} \frac{1}{\sqrt{1 - \frac{v^2}{c^2}}}$ \conseq Laufzeitunterschied \conseq optische Wegdifferenz der zwei Teilstrahlen 
$$\delta = c (\Delta_2 - \Delta_1) = 2 (\frac{l_2}{\sqrt{1 - \frac{v^2}{c^2}}} - \frac{l_1}{1 - \frac{v^2}{c^2}})$$ 
\conseq Drehung der Apparatur um $90^\circ$. Wie ändert sich die Intensität? \conseq Rolle von $l_1$ und $l_2$ wird vertauscht.
$$\delta' = c (\Delta_2' - \Delta_1') = 2 (\frac{l_2}{1 - \frac{v^2}{c^2}} - \frac{l_1}{\sqrt{1 - \frac{v^2}{c^2}}})$$
Wie muss der Apparat verstellt werden, wenn gedreht?
$$S = \delta' - \delta = 2(l_1 + l_2) (\frac{1}{1-\frac{v^2}{c^2}} - \frac{1}{\sqrt{1 - \frac{v^2}{c^2}}})$$
Annahme  $v \ll c$; $\frac{v}{c} \ll 1$
$$S \approxeq 2 (l_1 + l_2) (1 + \frac{v^2}{c^2} - (1 + \frac{1}{2} \frac{v^2}{c^2}) + \dots) = (l_1 + l_2) \frac{v^2}{c^2}$$
Sichtbares Licht $\lambda \approx 500 \mathrm{nm}$ und $c = 3 \cdot 10^8 \mathrm{m} \mathrm{s}^{-1}$ \conseq $l_1 + l_2 \approxeq 50 \mathrm{m}$ um Verschiebung um 1 1 Interferenzmaximum zu messen.
$M+M$ \conseq $\sim 10 \mathrm{m}$ mit Vielfachreflexionen \conseq wären sensitiv auf $\frac{v}{c}$ gewesen. Aber: \textbf{kein Effekt gemessen!}

\begin{folgerung*}~\\
	\begin{itemize}
		\item Lichtgeschwindigkeit überall gleich.
		\item Wenn es weiterhin Inertialsysteme geben soll (jetzt erst recht!). \conseq Galileo-Transformation falsch.
		\item[\conseq] Keine absolute Zeit. Kein absoluter Raum.
		\item[\conseq] ohne unendlich schneller Informationsaustausch. (kein absoluter Raum). Begriff der \textbf{Gleichzeitigkeit} an Synchronisation mittels Licht gebunden.
		Ruhendes System klar. Synchronisation möglich. Beim bewegten System komplizierter.
	\end{itemize}
\end{folgerung*}

\subsection{Einstein}: Festhalten an der Gleichberechtigung aller Bezugssysteme \conseq \textbf{Einsteins Postulate}
\begin{description}
	\item[Äquivalenzprinzip] Physikalische Gesetze und Experimente sind gleichförmig, geradlinig zueinander bewegten Systemen gleich.
	\item[Konstanz der Lichtgeschwindigkeit] Die Lichtgeschwindigkeit ist in allen System gleich ($=c$), unabhänig von der Bewegung der Quelle. 
\end{description}
\conseq Transformationsformeln für physikalische Größen.
Insbesondere Raum und Zeit, in dem als Grenzfall $v \ll c$ die Galilei-Transformation enthalten sein muss.

\subsubsection{Lorentz-Transformation}
Zwei Bezugssysteme $\Sigma$ und $\Sigma'$, wobei sich $\Sigma'$ mit $v$ gleichförmig geradlinig gegenüber $\Sigma$ bewegt. Die ausgezeichnete Achse ist $z$ bzw. $z'$ und die Zeit ist gleich.

gleichförmig, geradlinige Bewegung von $\Sigma'$ relativ zu $\Sigma$ mit $v$ ($\vec{v} \parallel \hat{z}$) bei $t = 0$, Beide Systeme identisch $\Sigma = \Sigma'$. 

Vergleich zweier Lcihtblitze, die zu $t=0$ im Ursprung von $\Sigma$ und $\Sigma'$ ausgesandt werden. $\Sigma: ~r = c t,~\Sigma:~r' = c t'$ Kugelwelle $c^2 + t^2 = r^2 = x^2 + y^2 + z^2$. Aber wegen der Konstanz der Lichtgeschwindigkeit $c^2t'^2 = x'^2 + y'^2 + z'^2 = r'^2$
\conseq Verknüpfung von Raum und Zeit.
\paragraph{Viervektoren}
$$x = (x^0, x^1, x^2, x^3) = (ct, \vec{x}\in\Re)$$
nicht euklidisch, sondern Elemente des Minkowski-Raums, hier
$$x^2 = (x^0)^2 - (x^1)^2 - (x^2)^2 - (x^3)^2 = {x^0}^2 - \vec{x}^2 = c^2t^2 - \vec{x}^2$$
\conseq gleiche Ausbreitung der Kugelwelle in $\Sigma$ und $\Sigma'$ bedeutet Invarianz des Vierervektorquadrats im Minkowski-Raum.
$$x^{~2} = x'^2$$
oder $$c^2 t^2 - x^2 - y^2 - z^2 = c^2 t'^2 - x'^2 - y'^2 - z'^2$$
Die gesuchte Transformation ist die \textbf{Lorentztransformation}. Offenbar ist die Wahl der Koordinatenachsen willkürlich \conseq \textbf{spezielle Lorentztransformation}, $\cdot $ Drehung = allgemeine Lorentztransformation.
Im folgenden immer entlang der z-Achse ($\vec{v} \parallel \hat{z}$). Beziehung zwischen $x$ und $x'$ notwendig linear, weil nur gleichförmig, geradlinige Bewegungen betrachtet.
$$x'^{\mu} = L^\mu_\nu x^\nu$$
$x'^\mu = \mu$-te Koordinate des Vierervektors $(x^0, x^1, x^2, x^3)$  

\paragraph{Index-Notation}
\begin{align*}
(\vec{F})_i &= (m \vec{a})_i &\rightarrow& F_i &=ma_i\\
S &= \vec{a}\cdot\vec{b} = \sum_{i=1}^{3}a_i b_i  &\rightarrow& a_ib_i &= S\\
(\vec{a})_i &= (M\vec{b})_i &\rightarrow& a_i &= \sum_{j=1}^3 M_{i\mathbf{j}} b_\mathbf{j} = M_{ij}b_j\\
\vec{a} &= L M \vec{b}  &\rightarrow& a_i &=L_{ij}M_{jk}b_k
\end{align*}
Vierervektoren $x^\mu, x$, $x^\mu$ oft sowohl Indexschreibweise als auch "`ganzer"' Vektor.\\
\conseq $x^\mu = (x^0, \vec{x})$ kontravarianter und $x_\mu = (x^0, -\vec{x})$ kovarianter.\\
Skalarprodukt $x_\mu y^\mu = \sum_{\mu=0}^{3} x_\mu y^\mu = x^0y^0 - \vec{x} \vec{y}$\\

%Weiter mit Lorentz...
Konvention: $\mu, \nu, \rho, \sigma = 0, 1, 2, 3$, 0 die Zeit, $i,j,k,l = 1,2,3$
und $\cos x y = \cos(x) y$
%
...
\intertext{hier $\nu = 0,1,2,3$}
\intertext{$x$,$y$-Richtung invariant, da Bewegung in z \conseq $x' = x$ und $y' = y$}
\begin{pmatrix}
ct' \\ x' \\ y' \\ z'
...
\end{align*}
\begin{align*}
\intertext{nur Vermischen der $ct$ und $z$-Komponente, nicht $x$ und $y$, folgende beide Gleichungen muss erfüllt sein}
(x')^2 &= (x)^2\\
...
\intertext{Damit hat man 3 Gleichungen für 4 Unbekannte \conseq 1 Parameter übrig. Verwendung des $sinh$ und $cosh$}
\end{align*}
\begin{align*}
...
-\sinh y & 0 & 0 & \cosh y 
\end{pmatrix}
\intertext{$y \in \Re$, Bedeutung? Zusammenhan zwischen $y$ und $v$ (Geschwindigkeit)?}
\intertext{Bewegung des Ursprungs von $\Sigma'$ in $\Sigma$?}
\end{align*}
\begin{align*}
x^3 &= v t = \frac{v}{c} c t = \frac{v}{c} x^0\\
x'^3 &= 0 = \cosh y \underbrace{x^3}_{\frac{v}{c} x^0 \text{wegen Bewegung des Ursprungs}} - \sinh y x^0 \text{~~~~Lorentz Transformation}\\
...
\intertext{$y$ ist die "`Rapidität"' oder auch "`verallgemeinerte Geschwindigkeit"'}
\end{align*}
\begin{align*}
...
\intertext{übliche Abkürzungen $\beta = \frac{v}{c}$; $\gamma = \frac{1}{\sqrt{1 - \beta^2}}$ \conseq $\cosh y = \gamma$; $\sinh y = \gamma \beta$}
\end{align*}
\begin{align*}
L &= \begin{pmatrix}
...
x' &= x, ~y'=y, ~z'=\gamma (z - \beta c t), ~ct' = \gamma (ct - \beta z)
\intertext{Dies sind die \textbf{Lorentztransformationsformeln}}
\intertext{Zeit wird mittransformiert, dies war bei der Galilei-Transformation nicht der Fall}
\end{align*}

\begin{bemerkung*}~\\
	\begin{enumerate}
		\item Grenzfall $v \ll c$ \conseq Galilei-Transformation
		\item $c$ is Maximalgeschwindigkeit (sonst $\gamma$ imaginär)
		\item Inverse Lorentztransformation $L^{-1} = L(v \to -v)$, klar, denn $\Sigma$ bewegt sich von $\Sigma'$ aus mit $-v$ entlang der $z$-Achse, genauer
		\begin{align*}
		\begin{pmatrix}
		\gamma & -\gamma \beta\\
		-\gamma \beta & \gamma
		...
		\end{pmatrix}
		\end{align*}
		\item $x^2 = x^0 x^0 - \vec{x} \vec{x} = {x^0}^2 - \vec{x}^2$. Neben $x^2$ \textbf{alle} Quadrate von Minkowski-4-Vektoren \textbf{invariant} unter Lorentztransformationen. Ebenso auch Skalarprodukte $x_\mu y^\mu = y^0 y^0 - \vec{x}\cdot\vec{y} = L_{\mu}^{~\nu} x_\nu = L^{\mu}_{~\rho}y^\rho =~$ "`Lorentz-Skalare"'
	\end{enumerate}
\end{bemerkung*}

\begin{folgerung*}[Folgerungen aus den Lorentztransformationen]
	\begin{description}~\\
		\item[\textit{(a)} Gleichzeitigkeit] $z' = \gamma (z - \beta c t)$\\ $ct' = \gamma (ct - \beta z)$ in zwei Systemen $\Sigma$ und $\Sigma'$ vergeht die Zeit unterschiedlich schnell. Gleichzeitigkeit?\\
		Im Bezugssystem $\Sigma$ zwei Ereigniss gleichzeitig bei $z_1$, $z_2$\\
		Im Bezugssystem $\Sigma'$: $ct_1' = \gamma (ct_1 - \beta z_1)$ und $ct_2' = \gamma (ct_2 - \beta z_2)$\\
		damit ist $c \Delta t' = c(t_1'- t_2') = \gamma\beta (z_2 - z_1) \neq 0$.
		Das Vorzeichen hängt vom Vorzeichen von $(z_2 - z_1)$ ab. Die Reihenfolge ist damit möglicherweise variabel.\\
		Vertauschung von Ursache und Wirkung?\\
		Sei in $\Sigma$ $t_2 > t_1$\\
		$\Sigma'$: $c\underbrace{(t'_2 - t'_1)}_{> 0 \text{, wenn kausal}} = \gamma (c (t_2 - t_1) - \beta (z_2 - z_1))$\\
		also $\gamma (c (t_2 - t_1) - \beta (z_2 - z_1)) > 0!$\\
		$\gamma > 0$, als $c (t_2 - t_1) > \beta (z_2 - z_1)$ immer, wenn $c(t_2 - t_2) > (z_2 - z_1)$ (da $\beta < 1$) und damit $c \Delta t > \Delta z$\\
		Also $c \Delta t \geq \Delta z$.\\
		Ein Lichtsignal kann Informationen zwischen $z$ und $z_2$ austauschen. \conseq Kausalität erhalten ($t'_2 > t_1$)\\
		Andernfalls $c \Delta t < \Delta z$\\
		Keine Informationsübertragung möglich \conseq auch die Reihenfolge der Ereignisse kann geändert werden.
	\end{description}
\end{folgerung*}
