...
\begin{align*}
	...
	L(r, \dot{r}, \phi, \dot{\phi}, z, \dot{z}) &= \frac12 m (\dot{r}^2 + r^2 \dot{\phi}^2 + \dot{z}^2) + \frac{\gamma}{\sqrt{\rho^2 + z^2}}
\end{align*}
...
\begin{align*}
	...
	p_\rho &= \ffpartial{L}{\dot r} = m \dot \rho\\
	p_z &= \ffpartial{L}{\dot z} = m \dot{z}
\end{align*}
...
\begin{align*}
	...
	\ffpartial{L}{\rho} &= m \rho \dot{\phi}^2 - \frac{\gamma}{\rho^2} &
	\Rightarrow m\ddot{\rho} - m \rho \dot{\phi}^2 + \frac{\gamma}{\rho^2} &\overset!= 0
\end{align*}

\paragraph{e)}
Annahmen: $\dot{\rho} = 0$ und $\ddot{\phi} = 0$\\
Aus der Annahme folgt offensichtlich auch $\ddot{\rho} = 0$, $\rho = \const$ und $\dot{\phi} = \const$
\begin{align*}
	m\ddot{\rho} - m \rho \dot{\phi}^2 + \frac{\gamma}{\rho^2} &\overset!= 0\\
	\xLeftrightarrow{\text{mit Annahme}} - m \rho \dot{\phi}^2 + \frac{\gamma}{\rho^2} &\overset!= 0\\
	m r^3 \dot{\phi}^2 - \gamma &= 0\\
    \rho &= \sqrt[3]{\frac{\gamma}{m {\dot{\phi}}^2}}
\end{align*}

\paragraph{f)}
Erinnerung: $p_\phi = m \dot{\phi} = l$
\begin{align*}
	r(l) &= \rho(m \dot\phi) = \sqrt[3]{\frac{m \gamma}{l^2}} = \sqrt[3]{m \gamma} l^{-\frac23}
\end{align*}
Wenn wir jetzt $l' = 2 l$ einsetzen
\begin{align*}
	\rho' &= \rho(l') = \sqrt[3]{m \gamma} 2^{-\frac23}l^{-\frac23} = 2^{-\frac23} r(l)
\end{align*}

\paragraph{g)}
\begin{align*}
	H &= \underbrace{m \dot{\rho}^2} + m \dot{\phi}^2 + m \dot{z}^2 - \frac12 m (\dot{\rho}^2 + \rho^2 \dot{\phi}^2 + \dot{z}^2) - \frac{\gamma}{\rho}\\
	&= \frac12 m \dot{\rho}^2 + m \dot{\phi}^2 (1 - \frac{\rho^2}{2}) + \frac12 m \dot{z}^2 - \frac{\gamma}{\rho}\\
	&= \frac12 \frac{p_\rho^2}{m} + \frac{p_\phi^2}{m} (\frac{1}{\rho^4} - \frac{1}{2 \rho^2}) + \frac12 \frac{p_z^2}{m} - \frac{\gamma}{\rho}
\end{align*}

\paragraph{i)}
\begin{align*}
	\{p_z, H\} &= \frac1{2m} \underbrace{\{p_z, p_\rho^2\}}_{=0} + \underbrace{\{p_z, \frac{p_\phi^2}{m} (\frac{1}{\rho^4} - \frac{1}{2 \rho^2})\}}_{=0} + \frac1{2m} \underbrace{\{p_z, p_z^2\}}_{=0} - \underbrace{\{p_z, \frac{\gamma}{\rho}\}}_{=0}\\
	&= 0
\end{align*}
