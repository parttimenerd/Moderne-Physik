...
damit $n_i = \frac{2a \cos \alpha}{\lambda}$
% % %
\conseq klarer Beweis der Wellennatur des Lichtes, siehe auch Wasser, Schall\\
\conseq Strukturuntersuchungen

\subsubsection{Photoeffekt}
(Lichtelektrischer Effekt)
vgl. Wikipedia

Licht mit Frequenz $\nu$ trifft mit Intensität $I$ auf Material und schlägt \circled{$e^-$} heraus. Wir bremsen die Elektronen mit einer Gegenspannung $U_g$ ab.
...
\begin{itemize}
	\item Photoeffekt tritt für Licht deshalb einer Grenzfrequenz $\nu_g$ auf.
	\item kinetische Energie $E_\text{kin}$ der \circled{$e^-$} nur von $\nu$ abhängig, \emph{nicht} von der Intensität des Lichtes. $\lightning$ klassischen Feld
	\item $\nu > \nu_g$ Anzahl der \circled{$e^-$} proportional zur Intensität
	\item Effekt sofort \conseq \circled{$e^-$} kann keine Energie aufsammeln
	\item[\conseq] \emph{Lichtquantenhypothese} (Einstein 1905): Licht = Ansammlung von Photonen mit Energie $h \nu$
\end{itemize}
\begin{align*}
...
Der im Experiment beobachtete \textbf{Welle-Teilchen-Dualismus} kann teilweise in der klassischen Mechanik wiedergefunden werden. \conseq Hamilton-Jacobi-Theorie. \conseq beschreibt die Ausbreitung von Punktteilchen durch sogenannte \textbf{Wirkungswellen}. Unter Zuhilfenahme der Erfahrung formuliert man schließlich die \textbf{Schrödingergleichung}, die sich nicht aus weiteren (bekannten) Prinzipien herleiten lässt.
...
\begin{bemerkung*}
	...
	Ist $\Psi(\vec{r}, t)$ wirklich eine "`Materiewelle"'?\\
	...
	wohl aber auf relative Phase zweier Lösungen (\conseq Interferenz)
	...
	Ansatz $e^{i\vec{k}\cdot \vec{r}} = e^{i k_x x} e^{i k_y y} e^{i k_z z}$\\
...
in Schrödingergleichung \conseq $(i \hbar \fpartial{t} - H) \Psi(\vec{r}, t) = 0 \rightarrow \int \d^3 \vec{k} \d \omega \tilde\Psi (\vec{k}, \omega) (\hbar \omega - \frac{\hbar^2 k^2}{2m}) e^{i(\vec{k}\cdot \vec{r} - \omega t)} = 0$\\
erfüllt durch $\tilde\Psi(\vec{k}, \omega) = \tilde\Psi (\vec{k}) \delta(w -w(\vec k))$ (mit $\int f(x) \delta(x - x_0) \d x = f (x_0)$)\\
also $\Psi(\vec{r}, t) = \int \d^3 \vec{k} \tilde\Psi (\vec{k}) e^{i(\vec{k}\cdot \vec{r} - \omega(\vec{k})t)}$\\
...
$$\omega(k) = \frac{\hbar k^2}{2m} \approxeq \frac{\hbar k_0^2}{2m} + \frac{\hbar k_0}{m}(k - k_0) + \dots \qquad \frac{\Delta k}{k_0} \ll 1$$
...
Wir schauen jetzt $$f(y) = \frac{\sin^2 y}{y^2} = \left(\frac{\sin y}{y}\right)^2$$ an: \href{http://www.wolframalpha.com/input/?i=\%28\\frac{\\sin+y}{y}\%29^2}{wolframalpha}\\
...
\begin{align*}
	H \psi(x) &= E \psi(x)\\
	\left(- \frac{\hbar^2}{2m} \ddd{^2}{x^2} + V(x)\right) \psi(x) &= E \psi(x)
\end{align*}
oder
$$\ddd{^2}{x^2} \psi(x) + \underbrace{\frac{2m}{\hbar^2}(E - V(x))}_{k^2(x)} \psi(x) = 0$$
$$\psi''(x) + k^2(x) \psi(x) = 0$$
...
\begin{bemerkung*}
	$k^2(x)$ \conseq siehe freie Lösung $\frac{\hbar^2 k^2}{2m} = E$
\end{bemerkung*}
...
\begin{bemerkung*}
	...
	$V(x)$ nur endliche Sprünge \conseq Knick in $\psi'(x)$ und $\psi'(x)$ stetig und $\psi(x)$ stetig.
\end{bemerkung*}
...
\begin{definition*}[Zustand]
	Der Zustand eines Systems ist, analog zu $\pi(t) = \{\vec{q}, \vec{p}, t\}$, hier in Quantenmechanik abstrakter Vektor $\ket{\Psi}$ (nicht $\vec{\Psi}$) aus \textbf{Hilbertraum}. Meist $\ket{\Psi} = \ket{\Psi(t)}$.
\end{definition*}
\begin{definition*}[Hilbertraum]
	$\infty$-dimensionaler Vektorraum mit Skalarprodukt und abzählbarer Basis.
	Verknüpfungen mit anderen Elementen oder komplexen Zahlen.
	% % % immer kleines \psi statt \Psi!!!!
	Hilbertraum $\mathcal{H}$.
	\begin{align*}
		\ket{\psi} + \ket{\vp} &\in \mathcal{H}\\
		c \ket{\psi} + d \ket{\vp} = \ket{c \psi + d \vp}
	\end{align*}
	$\exists$ \textbf{Nullelemente} $0, \ket{0}$
	\begin{align*}
		\ket{\alpha} + \ket{0} &= \ket{\alpha}\\
		0 \cdot \ket{\alpha} &= \ket{0}\\
		c \cdot \ket{0} = \ket{0}
	\end{align*}
	\textbf{Inverse} $\ket{\alpha} - \ket{\alpha} = \ket{\alpha} + \ket{-\alpha} = \ket{0}$\\
	Distributivgesetz $c (\ket{\alpha} + \ket{\beta}) = c \ket{\alpha} + c \ket{\alpha}$\\
	$(c+d) \ket{\alpha} = c \ket{\alpha} + d \ket{\alpha}$\\
	Sei $\{\ket{n}\}$ eine Basis $\ket{1}, \ket{2}, \ket{3}, \dots$ genau dann Basis wenn $\sum_n c_n \ket{n} = \ket{0}$ nur, wenn alle $c_n = 0$\\
	\textbf{Skalarprodukt} $\ket{\alpha}, \ket{\beta} \in \Co$
	$\ket{\alpha} \to \bra{\alpha}$ $\braket{\alpha | \beta} = c \in \Co$, $\braket{\alpha | \beta}* = \braket{\beta | \alpha}$, $\braket{\alpha | \beta + \gamma} = \braket{\alpha | \beta} + \bra{\alpha | \gamma}$, $c \braket{\alpha | \beta} = \braket{\alpha | c \beta} = \braket{c* \alpha | beta}$, $\braket{\alpha | \alpha} \geq 0$ wenn $\ket{\alpha} \neq 0$\\
	\textbf{Norm} $\| \alpha \| = \sqrt{\braket{\alpha | \alpha}}$\\
	\textbf{Orthogonalität} $\braket{\alpha | \beta} = 0$\\
	\textbf{Basis} $\ket{n}$ sei orthogonal $\braket{n | m} = \delta_{nm}$ am besten auch $\| n \| = 1$.\\
	beliebiger Zustan $\ket{\Psi}$ darstellen durch Basis mit $\braket{n | \Psi} = $ Projektion von $\ket{\Psi}$ und $\ket{n}$ mit Vollständigkeit $\sum_n \ket n \bra n = 1$\\
	$$\ket{\Psi} = \ket{\Psi} = \sum_n \underbrace{\ket{n}}_{\text{Basisvektor}} \overbrace{\braket{n | \psi}}^{\text{"`Länge"' von $\ket{\psi}$ in Richtung $\ket n$}}$$
	ebenso $$\ket{\psi} \to \begin{pmatrix}
	\braket{1 | \psi}\\ \braket{2 | \psi}\\ \vdots \\ \braket{n | \Psi}
	\end{pmatrix}$$
	Problem $n \to \infty$. \conseq kontinuierlicher Index $n \in \Re$. 
	$\braket{n | \psi} = \psi_n$ \conseq $\psi(n)$ = komplexe Funktion\\
	Anschluss der Wellenfunktion an die abstrakten Zustände $\ket{\psi}$ (Komponenten des Vektors) \conseq "`\textbf{Ortsdarstellung}"'.\\
	Vektor $\psi(x) = \braket{x | \psi}$ (1-dim.)\\
	bzw. $\psi(\vec{r}) = \braket{\vec{r} | \psi}$.
	Jetzt $\psi(\vec{r})$ Komponenten des $\ket{\psi}$ in der Basis $\ket{\vec{r}}$. $\ket{\vec{r}}$ = Eigenvektoren des Ortsoperators: $\underbrace{\hat{\vec{r}}}_{\text{\vspace{-10em}Operator}} \ket{\vec{r}} = \vec{r} \ket{\vec{r}}$.\\
	Dann \textbf{Skalarprodukt als Integral}
	\begin{align*}
		\braket{\psi | \vp} &= \braket{\psi | 1 | \vp} = \sum_n \braket{\psi | n} \braket{n | \vp}\\
		&= \sum_n \psi^\ast_n \vp_n \vp_n
		\intertext{bzw. im kontinuierlichem $n \to \vec{r}$}
		\braket{\psi | \vp} &= \int \d^3 \vec{r} \underbrace{\braket{\psi | \vec{r}}}_{\braket{\vec{r} | \psi}^\ast} \braket{\vec{r} | \vp} \text{mit $1 \int \d^3 \vec{r} \ket{\vec{r}} \bra{\vec{r}}$}\\
		&= \int \d^3 \vec{r} \psi^\ast(\vec{r}) \vp(\vec{r})
	\end{align*}
	\conseq Wellenfunktion $\psi(x)$ bzw. $\psi(\vec{r})$ ist eine konkruente Darstellung des abstrakten Zustandes $\ket\psi$\\
	Operatoren (meist lineare Operatoren)
	$$A \ket{\alpha} = \ket{\beta} = \ket{A \alpha}$$
	$A_1 = A_2$, wenn $A_1 \ket{\alpha} = A_2 \ket{\alpha}$ (für alle $\ket{\alpha}$).\\
	linear $(a A + b B) \ket{\alpha} = a A \ket{\alpha} + b B\ket{\alpha}$, $a,b \in \Co$, $A$, $B$ lineare Operatoren.
	$$A_2 A_1 \ket{\alpha} = A_2 \ket{A_1 \alpha} = A_2 (A_1 \ket{\alpha})$$
	adjungierter Operator
	$$\braket{\beta | A | \alpha} = \braket{\beta | A \alpha} = \braket{A^+ \beta | \alpha} = \braket{\alpha | A^+ | \beta}^\ast$$
	\conseq Eigenwertproblem
	$$A \ket{\alpha} = a \ket{\alpha}$$
	$A$ Operator, $\ket{\alpha}$ Eigenvektor, $a \in \Co$ Eigenwert\\
	analog zu Matrizen.\\
	hermitesche Matrix (selbstadjungiert) $A^+ = {A^t}^\ast A$
	$$\braket{\alpha | A | \alpha} = a \braket{\alpha | \alpha} = \dots = a^\ast \braket{\alpha | \alpha}$$
	\conseq $a \in \Re$
	\conseq hermitesche Operatoren haben reelle Eigenwerte
\end{definition*}
...
\begin{description}
	...
	\item[\href{http://physik.leech.it/pub/}{http://physik.leech.it/pub/}] Sammlung von Altklausuren von Physikstudenten. Die Klausuren zu Theo B (Analytische Mechanik) und Theo D (Quantenmechanik) sind teilweise zur Vorbereitung geeignet.
...
n-te Ableitung: $\frac{\d^n}{\d x^n} f(x)$
