...
\begin{description}
\item[$E>V$] $\to k^2 > 0 \Rightarrow \psi''(x) = -k^2 \psi(x)$ $\psi''$ hat immer ungleiches Vorzeichen von $\psi$. $\psi(x)$ ist zur x-Achse hin gekrümmt. $\Rightarrow$ Oszillation.
z.B. $V = \const$, $k=\const$ $\psi(x) = e^{\pm ikx}$
\[\psi'' = (\pm ik)(\pm ik)e^{\pm ikx} = -k^2 \psi(x)\]
allg: $\psi(x) = Ae^{ikx} + Be^{-ikx}$ mit $k=\sqrt{\frac{2m}{\hbar^2}(E-V)}$
