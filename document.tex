...
\paragraph{Annahme} im Hohlraum bilden sich stehende Wellen aus, im thermodynamischen Gleichgewicht entfallen auf jeden Freiheitsgrad $\frac12 k_B T$ Energie ($k_B =$ Boltzmann-Konstante).
...
Wir schauen jetzt $$f(y) = \frac{\sin^2 y}{y^2} = \left(\frac{\sin y}{y}\right)^2$$ an:
\begin{center}
\begin{tikzpicture}
\begin{axis}[xlabel=${y}$, ylabel=$(\frac{\sin y}{y})^2$, xtick=\empty, ytick=\empty, ymin = 0]
  \addplot[red, samples=500,domain=-3*pi:3*pi]{sin(deg(x)) ^2 / deg(x)^2};
  \end{axis}
\end{tikzpicture}
\end{center}
...
\begin{definition*}[Hilbertraum]
	...
	analog zu Matrizen.\\
	hermitesche Matrix (selbstadjungiert) $A^+ = A^{\top \ast} = A$
