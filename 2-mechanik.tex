...
\begin{equation*}
	\vec{F} = \dot{\vec{p}}
\end{equation*}
Falls $m = const.$ folgt mit $\dot{\vec{v}} = \vec{a}$
\begin{equation*}
	\vec{F} = m \vec{a}
\end{equation*}
\end{definition*}
...
$$\vec{F}_G = \gamma \frac{M m}{r^2} \hat{r}$$
 wobei $\hat{r} = \frac{\vec{r}}{|\vec{r}|}$ und $\gamma$ die Newtonsche Gravitationskonstante sind. Sofern der Abstand und eine der Massen, ohne Beschränkung der Allgemeinheit $M$, konstant ist, kann man die Formel zu $F = m g$ vereinfachen.Auf der Erde gilt $g \approx \SI{9.81}{\meter\per\second\squared}$.
...
Im Allgemeinen gilt für die Arbeit
\begin{equation*}
	W = \int_{\vec{r}} \vec{F} d\vec{r}
\end{equation*}
und die kinetische Energie
\begin{align*}
 	T &= \frac{1}{2} m \vec{v}^2\\
 	\dd{t}T &= \left(\dd{\vec{v}} T\right) \dd{t}\vec{v}\\
 	&= m\vec{v}\vec{a}\\
 	&= \vec{F}\vec{v}
\end{align*}
\begin{beispiel*}
	Wir betrachten einen Körper der auf einer beliebigen Bahn von Punkt A zu Punkt B und dann auf einer weiteren beliebigen Bahn von B zurück zu A bewegt wird.
	Dann gilt mit dem Gradienten 
	\begin{equation*}
		\vec{\nabla} = \left(
		\begin{matrix}
			\fpartial{x}\\\fpartial{y}\\\fpartial{z}
		\end{matrix}
		\right)
	\end{equation*}
	und  dem Potential $V$
	\begin{equation*}
		\vec{F} = - \vec{\nabla}V
	\end{equation*}
	Damit folgt für die Arbeit
	\begin{align*}
		W &= \oint\vec{F}d\rvec\\
		&= \int_A^B -\vec{\nabla}Vd\rvec + \int_B^A -\vec{\nabla}Vd\rvec\\
		&=-V(B) + V(A) - V(A) + V(B)\\
		&= 0
	\end{align*}
	Für konservative Kräfte gilt also: Die verrichtete Arbeit ist unabhängig vom Integrationsweg.\\
	Es folgt für die Gesamtenergie $E = T + V$
	\begin{align*}
		\dd{t} E &= \dd{t}T + \dd{t}V\\
		&=\vec{F}\vec{v}+ (\vec{\nabla}V)\dot{\rvec}\\
		&=\vec{F}\vec{v}+ (-\vec{F})\vec{v}\\
		&=0
	\end{align*}
\end{beispiel*}
...
Zeit in folgender Form: $$f_i(\vec{r}_1, \dots, \vec{r}_N, t) = 0\hspace{1cm} i = 1, \dots, p$$ 
...
$$ \frac{\partial f_i}{\partial t} = 0\hspace{1cm}i = 1, \dots, p$$
...
Dabei sind $Q_j$ die generalisierten Kräfte. Die Dimension der $Q_j$ ist nicht unbedingt Kraft, weil die Dimension von den $q_j$ selbst unklar ist. Jedoch ist die Einheit von $Q_j \cdot q_j$ natürlich die Energie.

\begin{bemerkung*}[Spezialfall: Konservative Systeme]
...
% TODO ist das wirklich die Lagrange-Gleichung 1. Art? 
...
Gegeben sei wieder ein konservatives System mit holonomen Zwangsbedingungen. Die Lagrange-Gleichung 2. Art lautet:
$$ \dd{t} \ffpartial{L}{\dot{q}_j} - \ffpartial{L}{q_j} = 0 \text{~für~} j= 1, \dots, s$$
Sie wird auch {\bf Euler-Lagrange-Gleichung} genannt.
...
Wobei diese Gleichung nur als Summe gültig ist, da die $\delta q_m$ nicht mehr unabhängig voneinander sind. Addiert man die beiden Gleichungen, folgt
\[
...
Hier sind Observablen lineare Operatoren $A$, $B$, $C$, \dots auf Hilberträumen. Z.B. $A,B,C = $ Matrizen auf gewöhnlichen Vektoren. Man definiert die Klammer jetzt als Kommutator\footnote{Der Unterschied zwischen $AB$ und $BA$ beschreibt die Symmetrie.} $[A,B] = AB - BA$. Konkret können Operatoren Matrizen sein. Hierbei ist die Nichtkommutativität z.B. von Drehungen bekannt.\footnote{Es spielt eine Rolle, ob man ein Objekt erst um die x-Achse um $90\degree$ und dann um die z-Achse um $90\degree$ rotiert, oder andersrum.} "`Reihenfolge der Operatoren ist relevant"'. Und damit ist natürlich $[A, B]$ nicht immer $0$.
Eine konkrete Formulierung der Quantenmechanik aufgrund dieser mathematischen Struktur ist:
\begin{enumerate}
	\item Physikalische Observable $A$ entspricht hermitischer, lineare Operator $A$ auf einem Hilbertraum.
	...
	$[q_i, q_j] = [p_i, p_j] = 0$.
	\item Hamilton-Funktion $H(\vec{q}, \vec{p}, t)$ wird zum Hamiltonoperator $H$, dessen Eigenwerte die möglichen Energiezustände sind.
