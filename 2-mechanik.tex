...
\begin{itemize}
	...
	$ \sum_{m = 1}^{3N} f_{im} \d x_m + f_{it} \d t = 0$\footnote{Wobei $f_{im} \coloneqq \ffpartial{f_i}{x_m}$ und $f_{it} \coloneqq \ffpartial{f_i}{t}$. Kürzer lautet die Einschränkung: $\d f = 0$.}
\end{itemize}
...
Es gibt $j = 3N - (\bar{p} - p)$ generalisierten Koordinaten (bei $\bar{p} - p$ holonomen Zwangsbedingungen). Die konkreten Koordinaten hängen diesen ab: $\vec{r}_i = \vec{r}_i(\vardots{q}{j}, t)$. Wie bereits erwähnt, sind die $q_j$ aber nicht alle unabhängig voneinander.

Damit gilt für nicht-holonome Bedingungen in $q_j$, wobei im Folgenden immer $i = 1, \dots, p$ ist\footnote{Wir setzen also $b_i = a_i = f_i \circ \vec{r}_i$, schreiben $f_i$ also in Abhängigkeit von $\vec{q}$ statt $\vec{r}$? Wieso dann im hinteren Teil ein $b_i$? Ich finde es verwirrend, woher auf einmal $a_i$ und $b_i$ herkommen.}:
\[
